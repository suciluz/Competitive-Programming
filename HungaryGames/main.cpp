...
typedef pair<int,int> pi; vector<pi> adj[maxn]; bool state[maxn];
    ...
    while(!pq.empty()) {
        ...
        for(auto &x: adj[u]) {
            if(-w+x.second < dis[x.first]) {
                ...
                pq.push({-dis[x.first],x.first}); state[x.first] = 1;
            }
...
int main() {
    ...
    for(int i=1,u,v,w; i<=m; i++) {
        scanf("%d %d %d",&u,&v,&w); adj[u].pb({v,w});
    }
    dijkstra(1);
...
/*
every node keep two values or do two times dijkstra (from start and from end)
*/
